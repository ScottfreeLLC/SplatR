...
# r.getstat
#
#    stat: the name of the statistic to extract
#    inputs: list of input (feature) names
#    models: a list of R model objects
#
# Description:
#
#    Extract all data from an R model summary into table
#    format; the function r.fixtable is a utility function
#    for detecting aliased variables that summary excludes
#
##############################################################

r.fixtable <- function(stable, v)
{
...
r.getstat <- function(stat, inputs, models)
   {
      svec <- numeric()
      sname <- stat$rname
      nvars <- length(inputs)
      ...
      if (!is.na(sindex)) {
         ...
         for (i in seq(along = models))
         {
            ...
            stable <- r.fixtable(stable, inputs)
